...
function draw() {
    ...
    for (var i = birds.length - 1; i >= 0; i--) {
        ...
        if (spectrum[i] > 100 || spectrum[i] < 20) {
          ...
          birds[i].maxspeed = ( map(spectrum[i], 0, 255, 1, 6 ) );
        }

        // birds[i].acceleration.add( createVector(birds[i], birds[i]) );
        birds[i].seek(center);
        birds[i].update();
        birds[i].display();
...
function Bird(x, y) {
    ...
    this.velocity = createVector(random(), random()); //just a random value...
...
//move the bird by adding velocity to location
